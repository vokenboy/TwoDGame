...
public class Projectile extends Entity implements Cloneable {
    ...
    public void update() {
        if (user instanceof Player shooter) {
            ...
            if (
                monsterIndex != 999 //collision with monster
            ) {
                shooter.damageMonster(
                    monsterIndex,
                    this,
                    attack * (1 + (shooter.level / 2)),
                    knockBackPower
                ); //attack : projectile's attack (2) // fireball dmg increases in every 2 levels
                generateParticle(
                    user.projectile,
                    gp.monster[gp.currentMap][monsterIndex]
                );
                alive = false;
            }
        } else {
