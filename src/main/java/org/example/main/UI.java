...
public class UI {
    ...
    private static final class EnchantOperation {
        ...
        EnchantOperation(
            ...
            int level,
            int value,
            ElementalDecorator.ElementType elementType,
            String summary
        ) {
            this.type = type;
            this.level = level;
    ...
    private static final class EnchantmentOption {
        ...
        EnchantmentOption(
            String title,
            List<EnchantOperation> operations,
    ...
    public void drawDialogueScreen() {
        ...
        if (npc.dialogues[npc.dialogueSet][npc.dialogueIndex] != null) {
            ...
            char characters[] = npc
                .dialogues[npc.dialogueSet][npc.dialogueIndex].toCharArray();
    ...
    public void drawInventory(Entity entity, boolean cursor) {
        ...
        for (int i = 0; i < entity.inventory.size(); i++) {
            ...
            //DISPLAY AMOUNT
            if (
                entity == gp.player && entity.inventory.get(i).amount > 1 //merchant npc's inventory cannot stack items
            ) {
                g2.setFont(g2.getFont().deriveFont(32f));
                int amountX;
                int amountY;
        ...
        if (cursor == true) {
            ...
            if (itemIndex < entity.inventory.size()) {
                ...
                for (String line : entity.inventory
                    .get(itemIndex)
                    .description.split("\n")) {
    ...
    public void drawTransition() {
        ...
        if (
            counter == 50 //the transition is done
        ) {
            counter = 0;
            gp.gameState = gp.playState;
    ...
    public void trade_buy() {
        ...
        if (itemIndex < npc.inventory.size()) {
            ...
            if (gp.keyH.enterPressed == true) {
                if (
                    npc.inventory.get(itemIndex).price > gp.player.coin //not enough coin
                ) {
                    subState = 0;
    ...
    public void trade_sell() {
        ...
        if (itemIndex < gp.player.inventory.size()) {
            ...
            if (gp.keyH.enterPressed == true) {
                if (
                    gp.player.inventory.get(itemIndex) ==
                        gp.player.currentWeapon ||
                    gp.player.inventory.get(itemIndex) ==
                    gp.player.currentShield //equipped items cant sell
                ) {
                    commandNum = 0;
                    subState = 0;
    ...
    private void performRandomEnchant(int itemIndex, Entity selectedItem) {
        Entity baseItem = selectedItem.originalItem != null
    ...
    private EnchantmentOption generateRandomOption(Entity baseItem) {
        ...
        int availableSlots = Math.max(
            1,
            baseItem.enchantmentSlots - baseItem.usedEnchantmentSlots
        );
        boolean baseHasElement = hasElementalMark(baseItem);
        EnchantmentOption option = createRandomOption(
            baseItem,
            availableSlots,
            baseHasElement
        );
        if (option != null) {
            option.detailLines.add(
                String.format(
                    "Slots used: %d/%d",
                    option.slotsUsed(),
                    baseItem.enchantmentSlots
                )
            );
        }
        return option;
    ...
    private EnchantmentOption createRandomOption(
        Entity baseItem,
        int maxSlots,
        boolean baseHasElement
    ) {
        ...
        while (remaining > 0 && operations.size() < 4 && attempts < 10) {
            EnchantOperation op = createRandomOperation(
                baseItem,
                remaining,
                !elementAdded
            );
        ...
        if (operations.isEmpty()) {
            operations.add(
                createOperation(
                    ...
                    baseItem,
                    Math.max(1, Math.min(3, 1 + rng.nextInt(3))),
                    null
                )
            );
        }
    ...
    private EnchantOperation createRandomOperation(
        Entity baseItem,
        int remainingSlots,
        boolean allowElement
    ) {
    ...
    private EnchantOperation createOperation(
        OperationType type,
        Entity baseItem,
        int level,
        ElementalDecorator.ElementType elementType
    ...
    private String buildSummary(
        ...
        String tier = "T" + level;
        return switch (type) {
            case DAMAGE -> String.format(
                "%s %s (+%d ATK)",
                type.label,
                tier,
                value
            );
            case DEFENSE -> String.format(
                "%s %s (+%d DEF)",
                type.label,
                tier,
                value
            );
            case LIFESTEAL -> String.format(
                "%s %s (+%d%% leech)",
                type.label,
                tier,
                value
            ...
            case ELEMENT -> String.format(
                "%s %s (%s)",
                type.label,
                tier,
                formatElement(elementType)
            );
        };
    }
    ...
    private String buildOptionTitle(List<EnchantOperation> operations) {
        ...
        for (EnchantOperation op : operations) {
            String highlight = op.type == OperationType.ELEMENT &&
                op.elementType != null
        ...
        String focus = highlights.isEmpty()
            ? ""
            : " (" + String.join(" & ", highlights) + ")";
        return prefix + " " + suffix + focus;
    }

    private Entity buildEnchantedItem(
        Entity baseItem,
    ...
    public void drawPlayerLife(Player uiPlayer) {
        Player targetPlayer = uiPlayer != null ? uiPlayer : gp.player;
        int x = gp.tileSize / 2;
        int y = gp.tileSize / 2;
        int i = 0;
        int iconSize = 32;
        ...
        //DRAW MAX LIFE (BLANK)
        while (i < targetPlayer.maxLife / 2) {
        ...
        while (i < targetPlayer.life) {
            g2.drawImage(heart_half, x, y, iconSize, iconSize, null);
            i++;
            if (i < targetPlayer.life) {
        ...
        for (int i = 0; i < gp.monster[1].length; i++) {
            ...
            if (monster != null && monster.inCamera() == true) {
                if (monster.hpBarOn == true && monster.boss == false) {
                    ...
                    if (
                        hpBarValue < 0 //Ex: You attack 5 hp to monster which has 3 hp. Monster's hp will be -2 and bar will ofset to left. To avoid that check if hpBarValue less than 0.
                    ) {
                        hpBarValue = 0;
                    }
                    ...
                    g2.fillRect(
                        monster.getScreenX() - 1,
                        monster.getScreenY() - 16,
                        gp.tileSize + 2,
                        12
                    );
    ...
    public void drawMessage() {
        ...
        for (int i = 0; i < message.size(); i++) {
            if (message.get(i) != null) {
                ...
                if (
                    messageCounter.get(i) > 150 //display 2.5 seconds
                ) {
                    message.remove(i);
    ...
    public void draw(Graphics2D g2) {
        this.g2 = g2;
        Player uiPlayer = gp.getActiveCamera();
